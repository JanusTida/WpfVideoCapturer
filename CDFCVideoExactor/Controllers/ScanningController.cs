...
namespace CDFCVideoExactor.Controllers
{
    public partial class ScanningController : IScanningController
    {
        ...
        public ScanningController(MainWindowViewModel mainWindowViewModel, IObjectDevice iObjectDevice, ObjectScanSetting objectScanSetting, IScanner scanner = null)
        {
            #region 实现初步验证步骤
            ...
            if (scanner == null)
            {
                ...
                switch (objectScanSetting.SelectedDeviceTypeEnum)
                {
                    ...
                    case DeviceTypeEnum.JVS:
                        scanner = new JVSScanner(iObjectDevice);
                        break;
    ...
    public partial class ScanningController
    {
        ...
        public bool Init()
        {
            ...
            res = scanner.Init(objectScanSetting.ScanMethod,
                    ...
                    ,objectScanSetting.SelectedVersionType.ID);
            //获得后缀名;
            ...
            if (objectScanSetting.ScanMethod == ScanMethod.Area)
            {
                try
                {
                    scanner.SetRegionsize(objectScanSetting.RegionSize * 1073741824);
                }
                catch (Exception ex)
                {
    ...
    public partial class ScanningController
    {
        ...
        private void UpdateAction()
        {
            ...
            try
            {
                loggerSetting = new CDFCLogger.Models.LoggerSetting
                {
                    ...
                    SerialNumber = iObjectDevice.DriveType == DriveType.PhysicalDevice ? ((Device)iObjectDevice).SerialNumber : iObjectDevice.DriveType == DriveType.Disk ? ((Partition)iObjectDevice).Device.SerialNumber : "",
                    Sign = iObjectDevice.DriveType == DriveType.Disk ? ((Partition)iObjectDevice).Sign.ToString() : "",
                    ImgPath = iObjectDevice.DriveType == DriveType.ImgFile ? ((ImgFile)iObjectDevice).Path : "",
                ...
                else
                {
                        ...
                        && objectScanSetting.ScanMethod == ScanMethod.EntireDisk))
                    {
                        ...
                        else
                        {
                            ...
                            scanningInfoDialogWindowViewModel.TitleWords = FindResourceString("CheckingFiles");
                    ...
                    scanningInfoDialogWindowViewModel.TitleWords = FindResourceString("ScanningDone");
                        ...
                        switch (scanner.ErrorType)
                        {
                            case 1:
                                ...
                                mainWindowViewModel.UpdateInvoker(() => {
                                    CDFCMessageBox.Show(FindResourceString("FailedToGetClusterSize"));
                                });
                                break;
                            case 3:
                                EventLogger.Logger.WriteLine("ScanningController->UpdateAction->扫描对象出错:未找到支持的厂商");
                                mainWindowViewModel.UpdateInvoker(() => {
                                    CDFCMessageBox.Show(FindResourceString("FailedToGetFirmInfo"));
                                });
                                break;
                        }
                    }
                    if (!Canceled)
                    {
                            ...
                            && scanningInfoDialogWindowViewModel.CurSectorCount != objectScanSetting.EndSector)
                        {
                            ...
                            scanningInfoDialogWindowViewModel.CurSectorCount = objectScanSetting.EndSector;// iObjectDevice.Size /(long)(iObjectDevice.SectorSize != 0 ? iObjectDevice.SectorSize : 512);
                        }
                    }
                    break;
                }
        ...
        public bool IniFilterViewModel(List<DateCategory> curCategories)
        {
            ...
            try
            {
                ...
                {
                    minDateNum = curCategories.Select(p => p.Videos.Min(q => q.StartDate == 0?q.EndDate:q.StartDate)).Min();
                    maxDateNum = curCategories.Select(p => p.Videos.Max(q => q.EndDate == 0?q.StartDate:q.EndDate)).Max();
                }
                ...
                {
                    mainWindowViewModel.UpdateInvoker.Invoke(() => {
                        mainWindowViewModel.VideoItemListViewerPageTopPartialPanelViewModel.
                        SetDateRange(minDateNum, maxDateNum);
                    });
                }
                ...
                Action SetChannelNos = () => {
                    ...
                    channelModels.Add(new ChannelNoModel { Info =FindResourceString("Total"), ID = -1 });
                    ...
                    string channelFormat = FindResourceString("ChannelNoFormat");
                    channels.ForEach(p => {
                        if (channelFormat.Contains("{0}")) {
                            channelModels.Add(new ChannelNoModel { Info = string.Format(channelFormat, p), ID = Convert.ToInt32(p) });
                        }
                        else {
        ...
        private void UpdateDialogInfo(List<DateCategory> categories)
        {
            ...
            mainWindowViewModel.UpdateInvoker.Invoke(() => {
                ...
                if (scanningInfoDialogWindowViewModel.StartDate != null && IsScanning)
                {
                    ...
                    if ((long)scanningInfoDialogWindowViewModel.ElapsedTime.TotalSeconds != 0)
                    {
                        var speed = (mainWindowViewModel.Scanner.CurrentSector - objectScanSetting.IniSector)
                        * (long)objectScanSetting.SectorSize / (long)scanningInfoDialogWindowViewModel.ElapsedTime.TotalSeconds;
        ...
        private void FakeUpdate()
        {
            ...
            while (percentage++ < 100)
            {
                ...
                scanningInfoDialogWindowViewModel.CurSectorCount = scanningInfoDialogWindowViewModel.TotalSectorCount / 100 * (long)percentage;
                Thread.Sleep(35);
            }
            //scanningInfoDialogWindowViewModel.CurSectorCount = mainWindowViewModel.Scanner.CurrentSector;
            //scanningInfoDialogWindowViewModel.CurPercentage = (byte)((scanningInfoDialogWindowViewModel.CurSectorCount * (long)iObjectDevice.SectorSize * 100 / iObjectDevice.Size));
        }
        /// <summary>
        /// 更新文件列表项;
        /// </summary>
    ...
    //            scanningInfoDialogWindowViewModel.FileCount = count;
    //            scanningInfoDialogWindowViewModel.CurSectorCount = mainWindowViewModel.Scanner.CurrentSector;
    //            scanningInfoDialogWindowViewModel.CurPercentage = (byte)((mainWindowViewModel.Scanner.CurrentSector * (long)iObjectDevice.SectorSize * 100 / iObjectDevice.Size));
    //            scanningInfoDialogWindowViewModel.TotalFileSize = fileSize;
    //        });
    //    }
    //    /// <summary>
