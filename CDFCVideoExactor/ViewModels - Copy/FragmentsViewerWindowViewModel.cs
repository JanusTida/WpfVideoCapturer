                ...
                return recoverFragmentCommand ??
                    (recoverFragmentCommand = new RelayCommand(
                        ...
                        && CDFCSetting.ScanSetting.VersionType.DeviceType != CDFCEntities.Enums.DeviceTypeEnum.WFS) {
                                CDFCMessageBox.Show("抱歉，当前版本下,此功能仅支持大华/WFS品牌");
                                return;
                        }
                        if (selectedFragment != null) {
                            var dialog = new VistaSaveFileDialog();
                            dialog.Title = "请选择输出位置";
                            if (dialog.ShowDialog() == true) {
                                ...
                                if (CDFCStatic.CMethods.CommonMethods.cdfc_common_read(
                                    ObjectDevice.Handle,
                                    selectedFragment.Fragment.StartAddress,
                                    ptrBuffer, selectedFragment.Fragment.Size,
                                    ptrSize, true)) {
                                    CDFCStatic.CMethods.CommonMethods.cdfc_common_write(saveFileStream.SafeFileHandle.DangerousGetHandle(),
                                        0, ptrBuffer, selectedFragment.Fragment.Size, ptrSize, false);
                                }
                                Marshal.FreeHGlobal(ptrBuffer);
                                Marshal.FreeHGlobal(ptrSize);
                                saveFileStream.Close();

                                if (CDFCMessageBox.Show("碎片保存完成，是否查看?", "保存完成", MessageBoxButton.YesNo) == MessageBoxResult.Yes) {
                                    try {
                                        ...
                                        var path = fileName.Substring(0,fileName.LastIndexOf("\\"));
                                          Process.Start("explorer", path);
                                            ...
                                            CDFCMessageBox.Show("打开文件夹错误!请自行打开。");
                                        }
                                    }
                                }
                            }
