...
namespace CDFCVideoExactor.ViewModels
{
    ...
    public partial class MainWindowViewModel
    {
        ...
        public string TitleWord {
            get {
                return titleWord + (SelectedEntranceType == EntranceType.Capturer ?
                    capturerSystemDes : SelectedEntranceType == EntranceType.MultiMedia ?
                    multimediaSystemDes : cpAndMSystemDes);
            }
            set {
                titleWord = value;
        ...
        public Visibility IsNotHomePage {
            get {
                return CurPageViewModel == videoObjectSelectorPageViewModel ? Visibility.Hidden : Visibility.Visible;
            }
        }
                ...
                else
                {
                    UpdateInvoker.Invoke(() => {
                        ...
                        if (CDFCMessageBox.Show("已找到相关记录，是否继续加载记录?", "已找到记录", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                        {
                            ...
                            try
                            {
                                CDFCSetting.ScanSetting.DeviceType = (DeviceTypeEnum)setting.DeviceTypeEnum;
                                CDFCSetting.ScanSetting.ExtensionName = setting.ExtensionName;
                                CDFCSetting.ScanSetting.IsMP4Class = setting.IsMP4Class == 1;
                                CDFCSetting.ScanSetting.ScanMethod = (ScanMethod)setting.ScanMethod;
                                CDFCSetting.ScanSetting.SectorSize = setting.SectorSize;
                                CDFCSetting.ScanSetting.StartingTime = DateTime.Now.ToString().Replace('\\', '-').Replace(':', '-');
                                CDFCSetting.ScanSetting.VersionType = new CDFCEntities.Models.VersionType((DeviceTypeEnum)setting.DeviceTypeEnum)
                                {
                                    ID = setting.DeviceVersionType
                                };
                                CDFCSetting.ScanSetting.DeviceTypeInfo = new CDFCEntities.Models.DeviceType { ID = 2, Info = "监控" };
