...
namespace CDFCVideoExactor {
    ...
    public partial class HelloWindow : Window {
        private HelloWindowViewModel vm;
        public HelloWindow() {
            InitializeComponent();
            vm = new HelloWindowViewModel(this.Dispatcher.Invoke);
            this.DataContext = vm;
            HelloController controller = new HelloController(vm);
            controller.Start();
        }

        //当窗体的可用状态发生变化时的处理;
        private void HelloWindow_IsEnabledChanged(object sender, DependencyPropertyChangedEventArgs e) {
            bool res = true;
