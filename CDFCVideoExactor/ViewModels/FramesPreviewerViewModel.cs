...
namespace CDFCVideoExactor.ViewModels {
    ...
    public partial class FramesPreviewerViewModel : ViewModelBase,IDisposable {
        ...
        public void EscapeToRow(VideoRow row) {
            ...
            if(targetBook == null) {
                ...
                try {
                    ...
                    EventLogger.Logger.WriteLine("FramesPreviewerViewModel->EscapeToRow:" + ex.Message);
                    CDFCMessageBox.Show(FindResourceString("FailedToRecoverFile"));
                    return;
                }
                #endregion
        ...
        public void AddPath(string path) {
            lock (PreviewImgs) {
                PreviewImgs.Add(new PreviewImg(path));
            }
        }
        //替换所有预览图像;
        public void UpdatePaths(List<string> imgPaths) {
            lock (PreviewImgs) {
                imgPaths.ForEach(p => {
                    PreviewImgs.Add(new PreviewImg(p));
                });
            }
        }
    ...
    public partial class FramesPreviewerViewModel {
        ...
        private void BuildPreviewImg(string targetFile,PreviewBook previewingBook, short page = 0) {
            ...
            for (int index = 0; index < 10; index++) {
                ...
                string imagePath = $"{curPath}previewFrames/{CurPreviewBook.Video.DateCategory.DeviceTypeEnum}"+
                    $"-{CurPreviewBook.Video.ChannelNO}-{CurPreviewBook.Video.StartDate}-{CurPreviewBook.Video.EndDate}-{page * 10 + index}-{ImgID++}.jpg";
