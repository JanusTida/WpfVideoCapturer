    ...
    public class HelloController : IHelloController {
        ...
        private bool CheckDongle() {
            if (mainWindow == null) {
                ...
                if(loginStatus == HaspStatus.ContainerNotFound) {
                    CDFCMessageBox.Show("抱歉，未找到加密狗!");
                    return false;
                }
                ...
                if (status == HaspStatus.StatusOk) {
                    ...
                    if (featureElems.Count() != 1) {
                        if (featureElems.Count() == 0) {
                            EventLogger.Logger.WriteLine("未找到序列号:" + entranceString);
                            CDFCMessageBox.Show("未找到对应序列号！请核实!");
                        }
                        else {
                                ...
                                if (totalTimeElem != null && startTimeElem != null) {
                                    ...
                                    if (Int32.TryParse(startTimeElem.Value, out startTimeNum)) {
                                        ...
                                        if (expirateTimeNum < nowNum) {
                                            ...
                                            CDFCMessageBox.Show("该密钥有效日期未达到此版本所要求，请继续使用原版本或与厂家联系升级!");
                                            return false;
                                        }
                                        else {
                                            return true;
