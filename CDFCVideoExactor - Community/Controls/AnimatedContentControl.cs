...
namespace CDFCVideoExactor.Controls {
    ...
    public class AnimatedContentControl : ContentControl {
        ...
        public static readonly DependencyProperty CurPageLevelProperty =
            DependencyProperty.Register(
                "CurPageLevel", typeof(byte),
                typeof(AnimatedContentControl)
                );

        public byte CurPageLevel {
            get {
                return (byte)GetValue(CurPageLevelProperty);
            }
            set {
                SetValue(CurPageLevelProperty, value);
            }
        }

        //private byte oldPageLevel = 0;
        ...
        protected override void OnContentChanged(object oldContent, object newContent) {
            var oldPageViewModel = oldContent as ViewModelBase;
            var newPageViewModel = newContent as ViewModelBase;
            if (m_paintArea != null && m_mainContent != null) {
                m_paintArea.Fill = CreateBrushFromVisual(m_mainContent);
                if (oldPageViewModel != null && newPageViewModel != null &&
                oldPageViewModel.PageLevel > newPageViewModel.PageLevel) {
                    BeginAnimateContentReplacement(true);
                }
                else {
        ...
        /// </summary>
        private void BeginAnimateContentReplacement(bool isBackForward = false) {
            ...
            if (!isBackForward) {
                newContentTransform.BeginAnimation(TranslateTransform.XProperty, CreateAnimation(this.ActualWidth/2, 0));
        ...
        private AnimationTimeline CreateAnimation(double from, double to, EventHandler whenDone = null) {
            IEasingFunction ease = new BackEase { Amplitude = 0.9, EasingMode = EasingMode.EaseOut };
    ...
    [TemplatePart(Name ="Animation")]
    public class AnimatedProgressBar:ProgressBar {
        protected override void OnValueChanged(double oldValue, double newValue) {
            //this.BeginAnimation(ProgressBar.WidthProperty, CreateAnimation(oldValue, newValue));
