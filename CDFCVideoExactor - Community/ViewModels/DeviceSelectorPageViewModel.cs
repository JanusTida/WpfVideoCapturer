...
namespace CDFCVideoExactor.ViewModels {
    public partial class DeviceSelectorPageViewModel:ViewModelBase {
        ...
        public ComObject ComObject {
            get {
                if(comObject == null) {
                    Logger.WriteLine("DeviceSelectorPage-> comObject对象为空!");
                }
                else {
                    if(comObject.Devices.Count == 0) {
                        Logger.WriteLine("DeviceSelectorPage-> comObject ->Devices对象为空!");
                    }
                }
                return comObject;
            }
                ...
                if(p.Partitions != null) {
                    ...
                    p.Partitions.ForEach(q => {
                        ...
                        try {
                            ...
                            Logger.WriteLine("加载" + q.Sign + "出错:" + ex.Message);
                        }
                    });
                }
                else {
        ...
        private void SureDoExecuted() { 
            if(selectedDeviceElem != null) {
                ...
                try { 
                    ...
                    Logger.WriteLine(@"DeviceSelectorPageViewModel->SureDoCommand出错,属性为空; 
                        selectedDeviceElem.InnerXml:" + selectedDeviceElem.InnerXml);
                    CDFCMessageBox.Show("抱歉,磁盘获取信息错误","错误",MessageBoxButton.OK);
                    return;
                }
                ...
                worker.DoWork += (sender, e) => {
                    ...
                    if (iObjectDevice == null) {
                        ...
                        if (iObjectDevice == null) {
                            Logger.WriteLine(@"DeviceSelectorPageViewModel->SureDoCommand出错,未获取到对象; 
                        selectedDeviceElem.InnerXml:" + selectedDeviceElem.InnerXml);
                            MainWindowViewModel.UpdateInvoker.Invoke(() => {
                                CDFCMessageBox.Show("抱歉,未能获取到对象", "错误", MessageBoxButton.OK);
                            });
                            return;
                        }
                    }
                    objectScanSetting = new ObjectScanSetting(iObjectDevice, MainWindowViewModel.SelectedEntranceType);
                    #endregion
                };
                //工作完成后处理;
